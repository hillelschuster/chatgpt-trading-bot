...
class CrossVenueValidateTest(unittest.TestCase):
    def test_collecting_before_minimum_complete_events(self):
    ...
    def test_positive_diversified_holdout_passes(self):
        rows = [row(i, coin="BTC" if i % 2 else "ETH") for i in range(220)]
        report, base, stress = validate(rows)
        self.assertEqual("PASS", report["verdict"])
        self.assertTrue(all(report["gates"].values()))
        self.assertEqual(report["holdout_attempts"], len(base)); self.assertEqual(len(base), len(stress))

    def test_concentrated_edge_rejected(self):
        report, _, _ = validate([row(i, coin="BTC") for i in range(220)])
        self.assertEqual("REJECT", report["verdict"])
        self.assertFalse(report["gates"]["positive_pnl_concentration_at_most_50pct"])

    def test_failures_are_charged_and_can_reject(self):
        rows = [row(i, coin="BTC" if i % 2 else "ETH") for i in range(220)]
        for i in range(160, 180):
            rows[i] = row(i, value=-.05, stress=-.05, coin="ETH", status="failed_attempt")
        report, _, _ = validate(rows)
        self.assertEqual("REJECT", report["verdict"])
        self.assertFalse(report["gates"]["failed_attempt_rate_at_most_10pct"])
