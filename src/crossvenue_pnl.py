...
def manifest_identity(path):
    ...
    return {
        ...
        "frozen_at_ms": int(manifest.get("frozen_at_ms") or 0),
...
def score_event(source, freeze=None):
    event = dict(source)
    if freeze:
        event["experiment_freeze"] = dict(freeze)
    ...
    if any(value is None for value in prices.values()):
        event["pnl_status"] = "invalid"; event["pnl_reason"] = "missing_or_invalid_executable_price"; return event
    if not entry.get("coordinated") or not exit_.get("coordinated"):
        event["pnl_status"] = "invalid"; event["pnl_reason"] = "uncoordinated_complete_event"; return event

    hl_rate, okx_rate = exact_rate(event, "hyperliquid"), exact_rate(event, "okx_swap")
    if hl_rate is None or okx_rate is None:
        event["pnl_status"] = "invalid"; event["pnl_reason"] = "missing_exact_realized_funding"; return event
    long_venue = (event.get("direction") or {}).get("long_venue")
    if long_venue not in ("hyperliquid", "okx_swap"):
        event["pnl_status"] = "invalid"; event["pnl_reason"] = "invalid_direction"; return event
    ...
    funding_return_pct = (50 * (okx_rate - hl_rate) if long_venue == "hyperliquid" else 50 * (hl_rate - okx_rate))
    base_cost, stress_cost = fixed_cost_pct(False), fixed_cost_pct(True)
    event.update({
        "pnl_status": "complete", "pnl_reason": None,
        "price_return_pct": price_return_pct, "funding_return_pct": funding_return_pct,
        "base_fixed_cost_pct": base_cost, "stress_fixed_cost_pct": stress_cost,
        ...
        "cost_model": {"capital_split": "50/50", "fills": "taker",
                       "hyperliquid_taker_rate": HL_TAKER_RATE, "okx_taker_rate": OKX_TAKER_RATE,
...
def summarize(rows, freeze=None):
    scored = [score_event(row, freeze) for row in rows]
    ...
    for row in attempts: by_coin[row.get("coin") or "UNKNOWN"] += float(row["base_net_return_pct"])
    positives = {coin: value for coin, value in by_coin.items() if value > 0}; positive_total = sum(positives.values())
    base = [float(row["base_net_return_pct"]) for row in attempts]
    stress = [float(row["stress_net_return_pct"]) for row in attempts]
    report = {"events": len(scored), "statuses": dict(sorted(counts.items())),
              "experiment_freeze": freeze,
              "complete_settled_events": len(complete), "scored_attempts": len(attempts),
              "base_fixed_cost_pct": fixed_cost_pct(False), "stress_fixed_cost_pct": fixed_cost_pct(True),
              ...
              "failed_attempt_rate": counts["failed_attempt"] / len(attempts) if attempts else None,
              "positive_pnl_concentration": max(positives.values()) / positive_total if positive_total else None,
              ...
              "profitability_claim_permitted": False, "minimum_complete_events": 200}
...
def main():
    ...
    args = parser.parse_args(); freeze = manifest_identity(args.freeze_manifest)
    rows, report = summarize(read_jsonl(args.path), freeze)
